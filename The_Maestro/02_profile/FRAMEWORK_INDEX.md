...
```mermaid
graph TD
    FP[First Principles] --> TD[Task Decomposition]
    TD --> MECE[MECE Analysis]
    MECE --> RACI[RACI Assignment]
    RACI --> ROUTE[Agent Routing]
    
    OODA[OODA Loop] --> CLASSIFY[Intent Classification]
    CLASSIFY --> CYNEFIN[Cynefin Categorization]
    CYNEFIN --> ROUTE
    
    ROUTE --> EXEC[Parallel Execution]
    EXEC --> SYNTH[Synthesis]
    
    MINTO[Minto Pyramid] --> SYNTH
    BLUF[BLUF] --> SYNTH
    SYNTH --> QA[Quality Check]
