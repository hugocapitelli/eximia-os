...
const CourseEdit: React.FC = () => {
   ...
   const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
         ...
         if (result.warning) {
            ...
            alert(`Imagem enviada! Nota: ${result.warning}`);
         } else {
            alert("Imagem de capa atualizada com sucesso!");
         }
      } catch (error: any) {
         console.error("Erro no upload:", error);
         const errorMsg = error?.response?.data?.detail || "Erro ao fazer upload da imagem. Verifique o console para mais detalhes.";
         alert(errorMsg);
   ...
   const handleAddChapter = async () => {
      ...
      try {
         ...
         console.error("Erro ao criar modulo:", e);
         alert("Erro ao criar modulo");
      }
   };

   const handleDeleteChapter = async (chapterId: string) => {
      if (!confirm("Tem certeza que deseja apagar este modulo e todo seu conteudo?")) return;
      try {
         await chaptersApi.delete(chapterId);
         await loadData();
      } catch (e) {
         console.error("Erro ao deletar:", e);
         alert("Erro ao deletar modulo");
      }
   };

   const handleDeleteCourse = async () => {
      if (!confirm("TEM CERTEZA? Esta acao e irreversivel. Todo o conteudo sera perdido.")) return;
      try {
                                          ...
                                          {contents.length === 0 ? (
                                             ...
                                             <div className="space-y-2">
                                                {contents.map((content) => (
                                                   <div
                                                      key={content.id}
                                                      className="flex items-center gap-3 p-3 bg-white rounded-lg border border-harven-border hover:border-primary cursor-pointer transition-all"
                                                      onClick={() => navigate(`/course/${courseId}/chapter/${chapter.id}/content/${content.id}/revision`)}
                                                   >
                                                      <span className={`material-symbols-outlined ${
                                                         content.type === 'video' ? 'text-red-500' :
                                                         content.type === 'audio' ? 'text-purple-500' : 'text-blue-500'
                                                      }`}>
                                                         {content.type === 'video' ? 'movie' : content.type === 'audio' ? 'headphones' : 'description'}
                                                      </span>
                                                      <div className="flex-1">
                                                         <p className="text-sm font-bold text-harven-dark">{content.title}</p>
                                                         <p className="text-[10px] text-gray-400 uppercase">{content.type}</p>
                                                      </div>
