...
const ContentRevision: React.FC = () => {
      ...
      const loadData = async () => {
          ...
          try {
              ...
              if (content.content_url) {
                  setFiles([{ name: content.title || 'Arquivo', type: content.type?.toUpperCase() || 'PDF' }]);
              }
  ...
  const handleReprocessAI = async () => {
      if (!contentData?.text_content) {
          alert('Não há conteúdo de texto para reprocessar. Faça upload de um documento com texto.');
          return;
      }
      ...
      try {
          ...
          if (!status.enabled) {
              alert('Serviço de IA não configurado. Configure OPENAI_API_KEY no backend.');
              return;
          }
          ...
          if (result.questions && result.questions.length > 0) {
              ...
              alert(`${newQuestions.length} novas perguntas geradas!`);
          } else {
              alert('Nenhuma pergunta foi gerada. Tente novamente.');
          }
      } catch (error: any) {
          console.error('Erro ao reprocessar:', error);
          alert('Erro ao reprocessar: ' + (error.message || 'Erro desconhecido'));
      } finally {
            ...
            <div className="flex-1 overflow-hidden bg-white">
               {isLoading ? (
                  ...
                  <div className="h-full overflow-y-auto custom-scrollbar">
                     ...
                     <div
                        className="p-6"
                        dangerouslySetInnerHTML={{ __html: formatExtractedText(contentData.text_content) }}
                     />
                  </div>
