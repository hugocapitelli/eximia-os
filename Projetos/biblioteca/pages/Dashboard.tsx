...
import { useReadingNow, useRecentlyAdded, useBooks } from '../hooks/useBooks';
import { BookOpen, TrendingUp, Brain, Loader } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const { books: readingNow, loading: loadingReading } = useReadingNow();
  const { books: recentlyAdded, loading: loadingRecent } = useRecentlyAdded(6);
  const { books: personalDev, loading: loadingPersonal } = useBooks({ category: 'Personal Development' });
  const { books: businessBooks, loading: loadingBusiness } = useBooks({ category: 'Business' });

  // Helper function to filter books by search
  ...
  const filteredReadingNow = filterBooks(readingNow);
  const filteredRecentlyAdded = filterBooks(recentlyAdded);
  const filteredPersonalDev = filterBooks(personalDev);
  const filteredBusiness = filterBooks(businessBooks);

  // Show loading state
  const isLoading = loadingReading || loadingRecent || loadingPersonal || loadingBusiness;
    ...
    return (
      <main className="flex-1 flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-600 dark:text-slate-400">Carregando sua biblioteca...</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main className="flex-1 px-4 md:px-10 lg:px-40 py-8 w-full max-w-[1600px] mx-auto pb-32">
      ...
      {filteredReadingNow.length > 0 && (
        <div className="mb-12">
          <div className="flex items-center justify-between px-2 pb-4 pt-2">
            <h2 className="text-slate-900 dark:text-white text-[22px] font-bold leading-tight tracking-[-0.015em] uppercase">
              Lendo Agora ({filteredReadingNow.length})
            </h2>
            ...
            {filteredReadingNow.map((book) => (
              ...
              >
                <div className="relative w-full aspect-[2/3] overflow-hidden rounded-lg">
                  ...
                  {/* Progress bar - would need reading_progress data */}
                ...
                <div>
                  ...
                  <p className="text-slate-500 dark:text-[#92a4c9] text-sm font-normal leading-normal">
                    {book.author} • {new Date(book.added_date).toLocaleDateString('pt-BR')}
                  </p>
                </div>
              </div>
            ))}
      ...
      {filteredRecentlyAdded.length > 0 && (
        <div className="mb-10">
          <h2 className="text-slate-900 dark:text-white text-[18px] font-bold leading-tight tracking-[-0.015em] px-2 pb-4 pt-2 uppercase opacity-90">
            Recém Adicionados
          </h2>
      ...
      {filteredPersonalDev.length > 0 && (
        <div className="mb-10">
          ...
          >
            ...
            <h2 className="text-slate-900 dark:text-white text-[18px] font-bold leading-tight tracking-[-0.015em] uppercase opacity-90 group-hover:text-primary transition-colors">
      ...
      {filteredBusiness.length > 0 && (
        <div className="mb-10">
          <div className="flex items-center gap-2 px-2 pb-4 pt-2">
            ...
            <h2 className="text-slate-900 dark:text-white text-[18px] font-bold leading-tight tracking-[-0.015em] uppercase opacity-90">
              Negócios & Estratégia
            </h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 p-2">
