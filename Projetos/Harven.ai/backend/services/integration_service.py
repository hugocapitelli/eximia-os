...
Uso:
    ...
    service = IntegrationService(supabase_client, settings)
...
class IntegrationService:
    ...
    def __init__(self, supabase_client, settings: Dict):
            ...
            for discipline in disciplines:
                ...
                for student_data in students:
                    ...
                    try:
                        ...
                        existing = self.supabase.table("users").select("id").eq("ra", ra).execute()
                        ...
                        if existing.data:
                            # Update
                            self.supabase.table("users").update(user_data).eq("ra", ra).execute()
                            records_updated += 1
                        else:
                            # Insert
                            user_data["id"] = str(uuid.uuid4())
                            user_data["password"] = ra  # Senha inicial = RA
    ...
    async def sync_disciplines_from_jacad(self) -> SyncResult:
        ...
        try:
            ...
            for disc in disciplines:
                ...
                try:
                    existing = self.supabase.table("disciplines").select("id").eq("code", code).execute()
                    ...
                    if existing.data:
                        self.supabase.table("disciplines").update(disc_data).eq("code", code).execute()
                        records_updated += 1
                    else:
                        disc_data["id"] = str(uuid.uuid4())
                        self.supabase.table("disciplines").insert(disc_data).execute()
                        records_created += 1
        ...
        try:
            ...
            for session in sessions:
                ...
                try:
                    ...
                    user_response = self.supabase.table("users").select("moodle_user_id, name").eq("id", session["user_id"]).execute()
            ...
            for session in sessions:
                ...
                if random.random() > 0.5:
                    ...
                    try:
                        ...
                        self.supabase.table("moodle_ratings").insert({
                            "id": str(uuid.uuid4()),
    ...
    async def get_moodle_ratings(self, filters: Dict = None) -> List[Dict]:
        ...
        query = self.supabase.table("moodle_ratings").select("*")
    ...
    async def _log_sync(self, result: SyncResult):
        ...
        try:
            self.supabase.table("integration_logs").insert({
                "id": str(uuid.uuid4()),
    ...
    async def get_logs(self, filters: Dict = None, limit: int = 50) -> List[Dict]:
        ...
        query = self.supabase.table("integration_logs").select("*")
    ...
    async def get_mappings(self, entity_type: str = None) -> List[Dict]:
        ...
        query = self.supabase.table("external_mappings").select("*")
    ...
    async def create_mapping(self, entity_type: str, harven_id: str,
        ...
        response = self.supabase.table("external_mappings").insert(mapping).execute()
