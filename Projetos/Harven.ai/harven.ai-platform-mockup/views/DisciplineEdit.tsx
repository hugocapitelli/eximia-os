...
const DisciplineEdit: React.FC = () => {
   ...
   const loadData = async () => {
      ...
      try {
         ...
         // Load courses in this discipline
         const coursesData = await coursesApi.listByClass(disciplineId!);
         setCourses(coursesData || []);

         // Load instructors for new course creation
   ...
   const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
         ...
         const errorMsg = error?.response?.data?.detail || "Erro ao fazer upload da imagem.";
         alert(errorMsg);
         loadData();
      } finally {
   ...
   const handleAddCourse = async () => {
      ...
      try {
         ...
         alert(`Erro ao criar curso: ${e.response?.data?.detail || e.message}`);
      }
   };

   const handleDeleteCourse = async (courseId: string) => {
      if (!confirm("Tem certeza que deseja excluir este curso e todo seu conteudo?")) return;
      try {
                  ...
                  {activeTab === 'courses' && (
                     <>
                        ...
                        <div className="flex justify-between items-center">
                           <div>
                              <h3 className="text-lg font-display font-bold text-harven-dark">Cursos da Disciplina</h3>
                              <p className="text-gray-500 text-sm">Gerencie os cursos que fazem parte desta disciplina</p>
                           </div>
                           <button
