   ...
   const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
         await adminApi.createAction({
            type: actionType,
            message: message,
            author: 'Admin' // TODO: Get logged user name
         });
         ...
         alert("Erro ao criar ação global");
      } finally {
      ...
      <div className="max-w-7xl mx-auto p-8 flex flex-col gap-8 relative">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {statCards.map((stat, i) => (
               <div key={i} className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-harven-border dark:border-gray-700 shadow-sm flex flex-col gap-1 group hover:border-primary/50 transition-colors cursor-pointer">
                  ...
                  {['JAN', 'FEV', 'MAR', 'ABR', 'MAI', 'JUN'].map((m, i) => (
                     <div key={i} className="flex flex-col items-center gap-2 group w-full">
                        <div className="w-full bg-harven-bg dark:bg-gray-700 rounded-t-lg h-32 relative overflow-hidden">
                           <div className="absolute bottom-0 w-full bg-harven-dark dark:bg-gray-900 group-hover:bg-primary transition-all rounded-t-lg" style={{ height: `${20 + i * 15}%` }}></div>
                        </div>
