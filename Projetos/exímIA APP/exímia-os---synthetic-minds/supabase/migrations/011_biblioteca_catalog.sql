-- ============================================================
-- MIGRATION 011: Biblioteca Catalog System
-- Purpose: Create book catalog, summaries, chapters, and related
-- tables for the biblioteca module
-- ============================================================

-- ============================================================
-- 0. CLEANUP - Drop existing views/tables if they exist
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.book_catalog (
  ...
  CONSTRAINT title_author_unique UNIQUE(title, author_name)
);

CREATE INDEX idx_book_catalog_title ON public.book_catalog(title);
CREATE INDEX idx_book_catalog_author ON public.book_catalog(author_name);
CREATE INDEX idx_book_catalog_google_id ON public.book_catalog(google_books_id);
CREATE INDEX idx_book_catalog_isbn ON public.book_catalog(isbn13, isbn10);

-- ============================================================
-- 2. BOOK SUMMARIES TABLE
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.book_summaries (
  ...
  CONSTRAINT summary_per_catalog UNIQUE(catalog_id, title)
);
...
-- ============================================================
-- 3. SUMMARY CHAPTERS TABLE
-- ============================================================

CREATE TABLE IF NOT EXISTS public.summary_chapters (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  CONSTRAINT chapter_order_unique UNIQUE(summary_id, chapter_number)
);
...
CREATE TABLE IF NOT EXISTS public.user_favorites (
  ...
  CONSTRAINT favorite_unique UNIQUE(user_id, catalog_id)
);
...
-- ============================================================
-- 6. SUMMARY READING PROGRESS TABLE
-- ============================================================

CREATE TABLE IF NOT EXISTS public.summary_reading_progress (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  summary_id UUID NOT NULL REFERENCES public.book_summaries(id) ON DELETE CASCADE,
  ...
  CONSTRAINT progress_unique UNIQUE(user_id, summary_id)
);
...
-- Catalog View with Summary Info
...
-- ============================================================
-- 9. AUTO-UPDATE TIMESTAMP TRIGGERS
-- ============================================================
...
-- book_catalog: Everyone can read, only admins can write
...
-- book_summaries: Published visible to all, all accessible to admins
...
-- summary_chapters: Published (via parent) visible to all, all to admins
...
-- user_favorites: Each user can only see their own
...
-- user_notes: Each user can only access their own
...
-- summary_reading_progress: Each user can only track their own
...
-- reading_preferences: Each user can only see/edit their own
...
-- Insert a few sample books (uncomment to use)
/*
INSERT INTO public.book_catalog (title, author_name, description, language, cover_url)
VALUES
  ('O Hábito do Sucesso', 'James Clear', 'Um livro sobre como pequenos hábitos levam a resultados extraordinários.', 'pt', 'https://example.com/atomic-habits.jpg'),
  ('O Milionário Instantâneo', 'Mark Fisher', 'Estratégias comprovadas para construir riqueza rapidamente.', 'pt', 'https://example.com/instant-millionaire.jpg'),
  ('Mindset: A Atitude Mental para o Sucesso', 'Carol S. Dweck', 'Como sua mentalidade determina seu sucesso.', 'pt', 'https://example.com/mindset.jpg')
ON CONFLICT (title, author_name) DO NOTHING;
*/
